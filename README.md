...
| 数据集名称     | 入度偏斜度 | 出度偏斜度 |
|----------------|-------|-------|
| **Web-Google**     | 73.82 | 5.05  |
| **soc-Pokec**      | 71.76 | 39.28 |
| **LiveJournal**    | 55.02 | 76.00 |

实验所用的三个数据集，均存在显著的数据倾斜。入度偏斜度高（>55），表明存在少数“枢纽节点”（hub nodes）被大量其他节点指向，典型如门户网站、热门社交账号。出度偏斜度在 LiveJournal 中高达 76，说明某些节点拥有极高的出链数量（如自动发布机器人、聚合页面），形成“超级源点”。
* **数据倾斜对 MapReduce 的影响：** MapReduce 模型本身对数据倾斜较为敏感，但其磁盘溢写机制和任务重试能力提供了基本容错性。 Shuffle 阶段热点键问题：在 PageRank 的 Reduce 阶段，所有指向同一高入度节点的贡献值需汇聚到同一个 Reducer。例如，在 LiveJournal 中，若某节点入度达数十万，则对应 Reducer 需处理海量中间键值对。然而，MapReduce 未因数据倾斜崩溃，尽管效率下降，但因其无状态、基于磁盘的设计，即使单个任务负载极高，仍可通过增加 Reducer 数量或启用推测执行缓解，系统具备一定鲁棒性。
* **数据倾斜对 Giraph 的影响：** Giraph 采用内存驻留的 BSP 同步模型，对数据倾斜容忍度较低。基于哈希的静态分区易将多个超级节点分配至同一 Worker，导致其内存超载、GC 频繁甚至 OOM，而其他 Worker 闲置。超级节点在单个 Superstep 中收发大量消息，引发网络拥塞，且 BSP 同步机制迫使全集群等待最慢节点。Giraph 的全内存设计虽提升计算效率，却使系统对数据倾斜更为敏感，一旦资源不足即导致作业失败，缺乏容错降级机制。
...
| **韩威如** | 51285903060        | 1.  Hadoop集群搭建与环境配置<br>2. 算法集群运行与测试<br>3. 系统资源监控数据收集                                                                             | 2 |
| **杜祥云**   | 51285903050 | 1. 完成迭代开销和数据规模扩展性测试分析<br>2. 完成PPT和讲稿制作<br>3. 录制介绍视频                            | 3 |
| **李怡萱** | 51285903059 | 1. 完成Giraph实现PageRank<br>2. Memory对比分析<br>3. 分析节点运行、崩溃情况                                                                 | 3 |
